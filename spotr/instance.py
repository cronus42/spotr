...
def open_port(client, instance, port):
    ...
    matching_rules = (
        x for x in group["IpPermissions"] if x.get("FromPort") == port and x.get("ToPort") == port
    )
            ...
            IpPermissions=[
                {
                    ...
                    "IpProtocol": "TCP",
                    "IpRanges": [
                        {"CidrIp": "0.0.0.0/0", "Description": "Spotr Jupyter Port"},
                    ],
                    "Ipv6Ranges": [
                        {"CidrIpv6": "::/0", "Description": "Spotr Jupyter Port"},
                    ],
                    "ToPort": port,
                }
            ],
